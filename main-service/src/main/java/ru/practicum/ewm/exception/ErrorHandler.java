...
public class ErrorHandler {
    ...
    public ApiErrorWrapper handleValidationException(MethodArgumentNotValidException e) {
        String message = e.getBindingResult().getFieldErrors().stream()
                .map(error -> "Field: " + error.getField() +
                        ...
                        ". Value: " + error.getRejectedValue()).collect(Collectors.joining("; "));

        log.warn("Ошибка валидации: {}", message);

        ApiError apiError = new ApiError("BAD_REQUEST",
                "Incorrectly made request.", message, LocalDateTime.now().format(FORMATTER));
    ...
    public ApiErrorWrapper handleValidationException(ValidationException e) {
        ...
        ApiError apiError = new ApiError("BAD_REQUEST",
                "Incorrectly made request.", e.getMessage(), LocalDateTime.now().format(FORMATTER));
